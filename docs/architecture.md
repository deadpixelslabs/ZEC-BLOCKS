...
NFT artwork can be regenerated by the published renderer from the specified collection inputs. Serving an image from a website does not itself prove or disprove valid ownership. Conversely, a deterministic image does not establish that all ownership evidence is publicly available.

## What operators can and cannot change

An operator can change a hosted interface, API, database projection or verification policy. A database administrator can affect the state displayed by that deployment. Those powers must not be described as eliminated simply because code is public.
...
Public code makes review and contributions possible. Some backend source and migrations are published, but the repository is not yet a complete, independently reproducible copy of the production service and event history. See the [source inventory](reference-implementation.md) and [roadmap](roadmap.md).

Sources: [marketplace operations](https://github.com/deadpixelslabs/ZEC-BLOCKS/blob/95648a4d7948dfb8c2466ed941718f405827621c/MARKETPLACE-OPERATIONS.md), [mining architecture](https://github.com/deadpixelslabs/test-zecblocks/blob/c5fc203dcf0557500f13530f49471788b1436a01/README.txt), [ZECS market backend](https://github.com/deadpixelslabs/ZEC-BLOCKS/blob/95648a4d7948dfb8c2466ed941718f405827621c/ZECS-MARKET-BACKEND.md).
