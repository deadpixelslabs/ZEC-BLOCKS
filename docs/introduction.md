...
The artwork is generated deterministically from collection inputs by the published renderer. An image preview is a representation of an NFT, not its ownership proof. A hosted image or cache can be unavailable without erasing a historical transaction, but independent recovery still requires the relevant event data and rules.

The project currently operates frontends, APIs and indexers. Those services affect discovery, availability and the state users see. Read [architecture](architecture.md) for these responsibilities and [source code](reference-implementation.md) for the files that implement them.
