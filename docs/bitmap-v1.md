...
ZEC BLOCKS artwork is generated from the NFT's source block hash, source height and token ID. The renderer returns a 600 × 600 SVG with a 24 × 24 logical grid. Identical valid inputs produce the same SVG; rendering does not consult a server or wallet.

The source height is `3488573 - token_id` for the configured collection. Verify the actual canonical source block before treating a preview as the artwork for a real NFT. A generated image does not prove ownership.

## Renderer code

Save this code as `renderer.mjs`. It is the deterministic renderer used by the reference tooling. It preserves the defined seed, palette and geometry rather than depending on a hosted PNG.
...
For a local check, save the following as `render-example.mjs` beside it and run `node render-example.mjs`:

```javascript
import { writeFileSync } from 'node:fs';
import { renderZb1Svg } from './renderer.mjs';

// Synthetic input, not a claim or a mainnet block record.
const svg = renderZb1Svg('11'.repeat(32), 3488572, 1);
writeFileSync('example.svg', svg);
```

Open `example.svg` in a browser to view the result. Changing the source hash changes the seed. Reproducing real artwork requires the real block hash and the same renderer, not merely an NFT number.

Artwork reproduction and ownership reconstruction are different checks. See [Genesis](genesis.md), [protocol rules](protocol-rules.md) and [state reconstruction](state-reconstruction.md).
